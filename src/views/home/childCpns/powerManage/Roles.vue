<template>
  <div class="roles">
    ...
    <el-card>
      <el-button type="primary" icon="el-icon-circle-plus-outline" @click="addRoles">添加角色</el-button>
        ...
        <el-table-column label="操作" width="300">
          <template v-slot="scope">
            <el-button
              ...
              icon="el-icon-edit"
              size="mini"
              @click="editRolesInfo(scope.row.id)"
            >编辑</el-button>
            <el-button
              type="danger"
              icon="el-icon-delete"
            ...
            <el-button
              ...
              size="mini"
              @click="allocationRights(scope.row)"
            >分配权限</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async deleteRolesInfo(id) {
      const confirmContent = await this.$confirm(
        "此操作将永久删除该角色, 是否继续?",
        "提示",
        {
    ...
    async confirmEditRights() {
      ...
      if (res.meta.status !== 200)
        return this.$message.error("更新角色授权失败！");
      this.$message.success("更新角色授权成功！");
      await this.getRolesList();
