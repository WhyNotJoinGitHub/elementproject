  ...
  export default {
    ...
    methods: {
      ...
      async allocationRights(role) {
        ...
        if (!role.children) {
          return this.selectRightsList.push(role.id)
        }
        for (let i = 0; i < role.children.length; i++) {
          let arri = role.children[i]
          if (!arri.children) {
            return this.selectRightsList.push(arri.id)
          }
          for (let j =0 ;j <arri.children.length;j++){
            let arrj = arri.children[j]
            if (!arrj.children) {
              return this.selectRightsList.push(arrj.id)
            }
            for (let k= 0; k<arrj.children.length;k++){
              let arrk = arrj.children[k]
              console.log(arrk);
              if (!arrk.children) {
                return this.selectRightsList.push(arrk.id)
              }
            }
          }
          // this.selectRightsList.push(role.children[i])
        }
